...
import { AuthenticationModalsContext } from "../App";
import { recoverPassword } from "../services/authentication.service";
import { ToasterError, ToasterSuccess } from "../helpers/toastr";

export function PasswordRecoveryModal(props: any) {
    const { hidePasswordRecoveryModal, showSignInModal } = useContext<any>(AuthenticationModalsContext);
    ...
    const onRecoverPasswordClick = () => {
        ...
        recoverPassword(login)
            .then(() => {
                ToasterSuccess("E-Mail de recuperação de senha enviado com sucesso.")
                showSignInModal();
            })
            .catch(error => {
                setIsLoading(false);
                if (error.response) {
                    ...
                    if (internalCode === 3) {
                    ...
                    else if (internalCode === 2) {
                        setLoginErrorMessage('E-Mail não encontrado.');
                    }
                        ...
                        ToasterError('Ocorreu um erro inesperado durante a recuperação de senha.');
                    }
                }
                else {
                    ToasterError('Ocorreu um erro inesperado durante a recuperação de senha.');
